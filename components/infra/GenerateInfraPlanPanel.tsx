...
// Kicks off the Phase 4 infrastructure provisioning planner. Mirrors
// GenerateSoftwarePlanPanel; only the endpoint + copy differ.
...
export function GenerateInfraPlanPanel({ projectId, failedMessage }: Props) {
  ...
  async function onGenerate() {
    ...
    try {
      const res = await fetch(
        '/api/projects/' + projectId + '/infra/plan/generate',
        { method: 'POST' },
      );
      ...
      setCoreState('error');
      setError(err instanceof Error ? err.message : 'Failed to plan the infrastructure.');
      setSubmitting(false);
    }
  }
      ...
      <div className="flex flex-col gap-4">
        <div>
          <h2 className="font-mono text-[10px] uppercase tracking-[0.4em] text-forge-cyan">
            provisioning plan · stage 02 · infrastructure
          </h2>
          <p className="mt-2 text-sm text-forge-dim">
            Turn the confirmed infrastructure spec into a four-layer provisioning
            DAG — private network + identity, data stores, compute workloads,
            observability — composed from a closed catalog of vetted, security-
            reviewed modules. The DAG is derived deterministically from your
            spec's resources and topology; a light LLM pass tightens each step's
            description. The planner NEVER authors raw provider, IAM, or network
            config.
          </p>
        </div>
        ...
        <div>
          ...
          >
            <span>{submitting ? 'Planning…' : 'Generate provisioning plan'}</span>
            <span
              aria-hidden
