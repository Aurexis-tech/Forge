// SystemSpec extraction pipeline.
//
// extractSystemSpec(): raw prompt (+ optional clarification answers /
//                      refinements) → validated SystemExtractionResult.
//
// Same multi-pass shape as the agent extractor (see lib/engine/spec/
// extract.ts): pass1 → if invalid, ONE repair retry → otherwise the
// caller surfaces open_questions / fails.
//
// All LLM calls flow through lib/engine/llm.ts so cost recording +
// governance happen uniformly across Phase 1 and Phase 2.
...
export class SystemExtractionError extends Error {
  ...
  constructor(message: string, opts?: { cause?: unknown; raw?: string }) {
    super(message);
    this.name = 'SystemExtractionError';
    this.cause = opts?.cause;
    this.raw = opts?.raw;
  }
}

export interface ExtractSystemSpecInput {
  rawPrompt: string;
...
export interface ExtractSystemSpecOutput {
...
export async function extractSystemSpec(
  input: ExtractSystemSpecInput,
): Promise<ExtractSystemSpecOutput> {
  const userMessage = buildSystemExtractionUserMessage({
  ...
  const govPass1: GovernanceScope = {
    ...input.governance,
    ref: (input.governance.ref ?? 'system.extract') + '.pass1',
  };
  const govRepair: GovernanceScope = {
    ...input.governance,
    ref: (input.governance.ref ?? 'system.extract') + '.repair',
  };
  ...
  try {
    repair = await complete({
      ...
      messages: [
        ...
        { role: 'user', content: buildSystemRepairUserMessage(parsed1.error) },
      ],
      governance: govRepair,
    });
  } catch (err) {
    if (err instanceof LLMError) {
      throw new SystemExtractionError(
  ...
  throw new SystemExtractionError(
    'Could not extract a valid SystemSpec after repair retry. Last validation error: ' +
      parsed2.error,
    { raw: repair.text },
  );
}
...
function tryParseExtraction(text: string): ParseOk | ParseErr {
  ...
  const validated = SystemExtractionResultSchema.safeParse(parsed);
