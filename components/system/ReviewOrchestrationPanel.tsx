...
// Approve-gate panel for the Phase 2 orchestration plan. Mirrors
// components/plan/ReviewPlanPanel.tsx — same confirm + refine rhythm
// but talks to the /system/plan/* endpoints and renders the
...
export function ReviewOrchestrationPanel({ projectId, plan }: Props) {
  ...
  async function onApprove() {
    ...
    try {
      const res = await fetch(
        '/api/projects/' + projectId + '/system/plan/approve',
        { method: 'POST' },
      );
  ...
  async function onRefine(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(
        '/api/projects/' + projectId + '/system/plan/refine',
        {
          method: 'POST',
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="font-mono text-[10px] uppercase tracking-[0.4em] text-forge-amber">
            orchestration · awaiting approval
          </h2>
          <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-forge-dim">
            phase 2 · review-only · build pipeline lands later
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 border-t border-white/5 pt-5">
          ...
          >
            {approving ? 'Locking…' : 'Approve orchestration'}
          </button>
        </div>
        ...
        {showRefine ? (
          ...
          >
            ...
            <textarea
              ...
              rows={3}
              disabled={refining}
              placeholder="e.g. 'the summarizer's tools should include llm_completion' or 'add a warning that the aggregator's inputs look thin'"
