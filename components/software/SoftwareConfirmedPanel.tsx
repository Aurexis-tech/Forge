// Confirmed-state panel for the Phase 3 SoftwareSpec. Mirrors the
// agent + system confirmed panels — locked, review-only badge,
// note that the build pipeline lands in a later phase.

import { GlassPanel } from '@/components/GlassPanel';
import { SoftwareSpecView } from './SoftwareSpecView';
import type { SoftwareSpec } from '@/lib/engine/software/spec';

export function SoftwareConfirmedPanel({ spec }: { spec: SoftwareSpec }) {
  return (
    <GlassPanel className="border-forge-amber/30 shadow-amber">
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-3">
          <span
            aria-hidden
            className="inline-block h-2 w-2 rounded-full bg-forge-amber shadow-amber"
          />
          <h2 className="font-mono text-[10px] uppercase tracking-[0.4em] text-forge-amber">
            software spec · confirmed
          </h2>
        </div>
        <SoftwareSpecView spec={spec} />
        <p className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.3em] text-forge-dim">
          locked · phase 3 is review-only · software code generation lands later
        </p>
      </div>
    </GlassPanel>
  );
}
