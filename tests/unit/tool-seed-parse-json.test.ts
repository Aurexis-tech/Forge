// Per-tool test — parse_json seed tool.
...
describe('parse_json — happy path', () => {
...
describe('parse_json — recovery paths', () => {
...
describe('parse_json — edge cases', () => {
...
describe('parse_json — determinism', () => {
  it('mock returns the same output for the same input across 25 calls', async () => {
    const text = 'pre ```json\n{"x":[1,2,3]}\n``` post';
    const first = JSON.stringify(
      await callTool({ name: 'parse_json', input: { text }, mode: MODE }),
    );
    for (let i = 0; i < 24; i++) {
...
describe('parse_json — examples parse against schemas', () => {
