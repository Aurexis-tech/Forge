...
// Approve-gate panel for the Phase 4 infrastructure provisioning plan.
// Mirrors ReviewSoftwarePlanPanel.tsx — same approve + refine
// semantics, same state transitions; renders the ProvisioningPlanView.
...
export function ReviewInfraPlanPanel({ projectId, plan }: Props) {
  ...
  async function onApprove() {
    ...
    try {
      const res = await fetch(
        '/api/projects/' + projectId + '/infra/plan/approve',
        { method: 'POST' },
      );
  ...
  async function onRefine(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(
        '/api/projects/' + projectId + '/infra/plan/refine',
        {
          method: 'POST',
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="font-mono text-[10px] uppercase tracking-[0.4em] text-forge-amber">
            provisioning plan · awaiting approval
          </h2>
          <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-forge-dim">
            phase 4 · plan-only · provisioning lands later
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 border-t border-white/5 pt-5">
          ...
          >
            {approving ? 'Locking…' : 'Approve provisioning plan'}
          </button>
        </div>
        ...
        {showRefine ? (
          ...
          >
            ...
            <textarea
              ...
              rows={3}
              disabled={refining}
              placeholder="e.g. 'tighten the ingest_worker description to mention idempotency' or 'add a warning about ephemeral lifecycle wiping the database'"
